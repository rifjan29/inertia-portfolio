...
export default function List ({ auth,session }) {
    ...
    const deleteData = (e) => {
        destroy(route('kategori-artikel.destroy',e));
    }
    return (
        <Authenticated
            user={auth.user}
                ...
                <div className='p-4 flex justify-between items-center'>
                    ...
                    <div>
                        <Breadcrumb aria-label="Solid background breadcrumb example" className="bg-gray-50 px-5 py-3 dark:bg-gray-800">
                            ...
                            <Breadcrumb.Item>Kategori Artikel</Breadcrumb.Item>
                        </Breadcrumb>
                    </div>
                </div>
            </div>
                ...
                <div className='p-4 bg-gray-50 dark:bg-gray-800 h-fit'>
                        <div className="flex flex-col md:flex-row items-center justify-end space-y-3 md:space-y-0 md:space-x-4 p-4 mb-4">
                            ...
                            <div className="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
                                <Button color='blue' href={route('kategori-artikel.create')} >
                                    <svg
                        ...
                        <Table.Body className="divide-y">
                            {kategori.data.map((data, index) => (
                                <Table.Row className="bg-white dark:border-gray-700 dark:bg-gray-800" key={index}>
                                    <Table.Cell>{index + kategori.from}</Table.Cell>
                                    ...
                                    <Table.Cell>
                                        <a href={route('kategori-artikel.edit',data.id)} className="font-medium text-cyan-600 hover:underline dark:text-cyan-500">
                                        Edit
                                        </a>
                                        <a href="#" onClick={() => {
                    ...
                    <div className='flex justify-end'>
                        <Pagination className="mt-6" links={kategori.links} />
                    </div>
                </div>
            </div>
        </Authenticated>

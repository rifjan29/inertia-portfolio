...
export default function Edit ({auth,session}) {
    const { dataExperience } = usePage().props;
    const [editorContent, setEditorContent] = useState(dataExperience.deskripsi);
    const { data, setData,post, errors,processing  } = useForm({
        title: dataExperience.title,
        perusahaan: dataExperience.perusahaan,
        deskripsi: dataExperience.deskripsi,
        tanggal_mulai: dataExperience.tanggal_mulai,
        tanggal_akhir: dataExperience.tanggal_akhir,
        _method: "",
    });
    ...
    const submit = (e) => {
        e.preventDefault();
        data._method = 'put'
        post(route('experience.update',dataExperience.id));
    }
    return (
        <Authenticated
            user={auth.user}
                ...
                <div className='p-4 flex justify-between items-center'>
                    ...
                    <div>
                        <Breadcrumb aria-label="Solid background breadcrumb example" className="bg-gray-50 px-5 py-3 dark:bg-gray-800">
                            ...
                            <Breadcrumb.Item>Edit Experience</Breadcrumb.Item>
                        </Breadcrumb>
                    </div>
                </div>
            </div>
